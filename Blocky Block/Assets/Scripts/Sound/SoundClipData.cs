...
{
    [CreateAssetMenu(menuName = "Sound Configs/Sound Clip Data %#m")]
    public class SoundClipData : ScriptableObject
    {
        ...
        public AudioClip GetClip()
        {
            ...
            if (this.isUseSeasonalClip)
            {
                if (seasonalClips.Count > 0)
                {
                    int index = Random.Range(0, seasonalClips.Count);
                    return seasonalClips[index];
                }
                return seasonalClips[0];
            }
